...
{
void joints_cubic_polynomial(moveit::planning_interface::MoveGroupInterface& interface,
                             moveit::planning_interface::MoveGroupInterface::Plan plan, std::vector<double> target)
{
  ...
  for (double t = 0.0; t < 1.0; t += 0.01)
  {
    ...
    {
      double position =
          coefficients[i][0] + coefficients[i][1] * t + coefficients[i][2] * pow(t, 2) + coefficients[i][3] * pow(t, 3);
